...
int main()
{
    int a, n, num;
    float x_value;
    float f_x=0, l_x0=0, l_x1=0, l_x2=0;
    ...
    l_x0 = ((x_value-x[a-1])/(x[a-2]-x[a-1])) * ((x_value-x[a])/(x[a-2]-x[a]));
    l_x1 = ((x_value-x[a-2])/(x[a-1]-x[a-2])) * ((x_value-x[a])/(x[a-1]-x[a]));
    l_x2 = ((x_value-x[a-2])/(x[a]-x[a-2])) * ((x_value-x[a-1])/(x[a]-x[a-1]));

    //finding the value of y(x) or f(x) at x=taken from users
    f_x = y[a-2]*l_x0 + y[a-1]*l_x1 + y[a]*l_x2;
    ...
    printf("\n\nWhen x = %.4f,", x_value);
    printf("\ny(%.2f) = y(%d) X l0(%d) + y(%d) X l1(%d) y(%d) X l1(%d)\n", x_value, a-2, a-2, a-1, a-1, a, a);
    printf("        = %.3f X %.3f + %.3f X %.3f + %.3f X %.3f \n", y[a-2], l_x0, y[a-1], l_x1, y[a], l_x2);
    printf("        = %.3f\n", f_x);
